...
module.exports = {
  name: "skip_n",
  description: "Skips to a specific episode",
  execute(message, args, stats_db, viewer_db, video_db) {
    ...
    client.guilds.cache
      ...
      .then((connection) => {
        ...
        if (client.episodes.url.length < args[0] || args[0] < 0) {
          client.guilds.cache.get(client.config.serverId).channels.cache.get(client.config.textChannelId).send("O número do episódio não é válido");
          client.user_commands.get("list").execute(message, args);
          return;
        }
        song = client.episodes.url[args[0] - 1];
